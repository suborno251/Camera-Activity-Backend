...
export async function seed(knex: Knex): Promise<void> {
  ...
  // ── 6 Workstations
  await knex('workstations').insert([
  ...
  // ── Helper to generate events for a worker/station pair
  const generateEvents = (
    ...
    for (let h = 0; h < hours; h++) {
      ...
      // Working event
      events.push({
        timestamp:      workStart.toISOString(),
        worker_id:      workerId,
      ...
      // Product count event
      events.push({
        timestamp:      workEnd.toISOString(),
        worker_id:      workerId,
        workstation_id: stationId,
        event_type:     'product_count',
        confidence,
        count: pattern === 'high' ? 35 : pattern === 'medium' ? 28 : 18,
      });
    }
  ...
  // ── Generate events for each worker/station pair
  const allEvents = [
  ...
  // ── W5 also gets a few absent events (low performer)
  allEvents.push(
